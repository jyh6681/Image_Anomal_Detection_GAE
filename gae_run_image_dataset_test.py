...
def train_dominant(args,normal_dataloaders,train_loader,test_loader,model):
    ...
    for epoch in range(args.epoch):
        ...
        for data in normal_dataloaders:
            ...
            labels = [tmp_data.y for tmp_data in data]
            labels = torch.cat(labels).cuda()
            X_hat = model(data)             ###################input to model##############
            mask,X_hat = model(data)             ###################input to model##############
            loss = nn.L1Loss()(mask,labels)
            loss.backward()
            optimizer.step()
        ...
        for data in test_loader:
            ...
            if min_score > val_loss.item():
                min_score = val_loss.item()
                state = {'net': model.module.state_dict(), 'optim': optimizer.state_dict(), 'epoch': epoch + 1,"lr":optimizer.state_dict()['param_groups'][0]['lr']}  ##最好保存lr
                torch.save(state, PATH + "eye_model_hid_"+str(args.hidden_dim)+".pth")
    return X_hat,mask


def test_dominant(args,test_loader,model,out_index=0):
    PATH = "/home/jyh_temp1/Downloads/GraphLocalDenoising/eye_image/"
    model_state_dict = torch.load(PATH + "eye_model_hid_"+str(args.hidden_dim)+"_normal_nopos.pth")['net']
    new_model_state_dict = {}
    for key, value in model_state_dict.items():
        if 'module.' in key:
            new_model_state_dict[key.replace('module.', '')] = value
        else:
...
normal_dataloaders = DataLoader(normal_dataset, batch_size=500,num_workers=10)
...
out,mask,label = test_dominant(args,val_dataloaders,model,out_index=out_index)
if args.mask_stored==1:
    ...
    recon_img = torch_vector2image(out,h,w,ksize,stride,padding)
    ...
    cv2.imwrite(dir_pth+"recon_"+str(args.epoch)+"out_"+str(out_index)+".png",recon_img)
    ...
    center2 = (88,219)
    # 在图像中心绘制半径为200的白色圆
    circle_fundus = cv2.circle(np.zeros((440,440)), center, 205, 1, -1)
    circle_disc = cv2.circle(np.ones((440,440)), center2, 50, 0, -1)
    mask_img = circle_fundus*mask_img
    mask_img = circle_disc*mask_img
    mask_img = mask_img/np.max(mask_img)
    thres_list =[0.3,0.4,0.2,0.15]
    ...
    for thres in thres_list:
        ...
        # 输出AUC值
        print("index:",out_index,"AUC值:", auc,ap,"thres:",thres)
